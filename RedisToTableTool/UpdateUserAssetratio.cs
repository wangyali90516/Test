...
{
    public partial class UpdateUserAssetratio : Form
    {
        ...
        public UpdateUserAssetratio(LoadAppSettings loadAppSettings, CloudTableClient tableClient)
        {
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (string t in userIds)
                {
                    ...
                    List<UserAssetRatio> userAssetRatios = deptTable.ExecuteQuery(queryDeptInfo).Where(x => x.OriginalUserAssetRatioId == null).ToList();
                    //List<UserAssetRatio> userAssetRatios = deptTable.ExecuteQuery(queryDeptInfo).ToList();
                    foreach (UserAssetRatio item in userAssetRatios)
                    {
                        ...
                        {
                            //日志
                            Logger.LoadData(@"UpdateOriginalUserAssetRatioIdLogs\logInfo.txt", modifyUserAssetRatioRequest.ToJson());
                            Logger.LoadData(@"UpdateOriginalUserAssetRatioIdLogs\logInfo.txt", "\r\n");
                        }
                    }
            ...
            {
                this.btn_handeluserids.Enabled = true;
                Logger.LoadData(@"UpdateOriginalUserAssetRatioIdLogs\Error.txt", ex.Message + "----" + ex.StackTrace);
            }
        }
        ...
        private void UpdateUserAssetratio_Load(object sender, EventArgs e)
        {
        }
    }
}